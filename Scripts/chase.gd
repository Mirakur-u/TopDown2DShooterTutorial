...
const CHANCE_TO_STOP_CHASING = .45


@onready var navigation_agent_2d: NavigationAgent2D = $"../../NavigationAgent2D"
@onready var sprite_2d: Sprite2D = $"../../Sprite2D"
@onready var random_target_chase_timer: RandomTimer = $RandomTargetChaseTimer
...
func exit():
	random_target_chase_timer.stop()
	sprite_2d.texture = texture_default
	owner.current_speed = owner.wandering_speed



func _on_random_target_chase_timer_timeout() -> void:
	if try_to_stop_chasing():
		return
	start_chasing()

func start_chasing():
	random_target_chase_timer.setup()
